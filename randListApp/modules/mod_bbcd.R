# =============================================================================
# Module: Bayesian Biased Coin Design (BBCD)
#
# bbcdPar(N, a, groups) — 2-arm only
# genSeq(par, seed = seed)
# =============================================================================

mod_bbcd_ui <- function(id) {
  ns <- NS(id)
  ...
  layout_sidebar(
    sidebar = sidebar(
      ...
      numericInput(ns("a"), "Prior parameter (a \u2265 0):",
    ...
    div(
      ...
      h2(class = "rd-method-title", "Bayesian Biased Coin Design"),
      span(class = "rd-method-badge badge-coral", "2-arm only"),

      p(class = "rd-description",
        "The Bayesian Biased Coin Design (Atkinson, 2012) derives the allocation
        probability from a Bayesian decision-theoretic framework. The parameter
        a \u2265 0 plays the role of a prior and controls the trade-off between
        randomness and balance. Like the Adjustable Biased Coin Design, setting
        a = 0 yields Complete Randomization; larger values produce sequences
        with stronger balance properties."),
...
mod_bbcd_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generate, {
      ...
      par <- bbcdPar(N = n, a = a, groups = c(g1, g2))
    ...
    output$download <- downloadHandler(
      filename = function() "bbcd_randomization.html",
